...
<?php if(!isset($_SESSION['username'])): header("location: logout.php");?>
  ...
  <div class="maincontent">
    <button class="topbutton">Assign Inventory</button>
    <hr class="new1">
    ...
    &nbsp &nbsp &nbsp &nbsp &nbsp <input type="submit" name="projectup-btn" class="btn" value="Assign">
    </form>
...
<?php
        if(isset($_POST['projectup-btn'])) {
          ...
          try {
            if($projectid!=""){
              $SQLInsert = "INSERT into assigninventory(projectid, inventoryname , username) VALUES ('$projectid','$inventoryname','$username')";

            $statement = $conn->prepare($SQLInsert);
            $statement->execute();
            echo '<script>alert("Inventory assigned Successfully. Press Back to see it in table")</script>';
            }
          }
