...
<?php if(!isset($_SESSION['username'])): header("location: logout.php");?>
  ...
  <div class="maincontent">
    <button class="topbutton">Update Role</button>
    <hr class="new1">
...
<?php
        if(isset($_POST['projectup-btn'])) {
            ...
            try {
              if($projectid!=""){
                $SQLInsert = "UPDATE assign SET post = '$assignrole' WHERE id = $projectid";
  
              $statement = $conn->prepare($SQLInsert);
              $statement->execute();
              echo '<script>alert("Updated Successfully. Press Back to see it in table")</script>';
              }
            }
